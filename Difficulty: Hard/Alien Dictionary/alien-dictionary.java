...
class Solution {
     public String findOrder(String[] dict, int N, int K) {
        ...
        // Step 2: Create the graph by comparing adjacent words
        int[] indegree = new int[K]; // Store indegree for each character
        ...
        // Add all characters with zero indegree to the queue
        for (int i = 0; i < K; i++) {
            if (indegree[i] == 0) {
                queue.add(i);
            }
        ...
        // Step 4: Check if the order contains all K characters
        if (order.length() != K) {
            return ""; // Invalid order as not all characters are included
        }
